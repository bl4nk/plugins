...
public OnPluginStart()
{
    ...
    do
    {
        if (StrEqual(name, "mp_bonusroundtime", false)) {
            convar = FindConVar(name);
            SetConVarBounds(convar, ConVarBound_Upper, true, 30.0);
        }
        
        if (isCommand || !(flags & FCVAR_DEVELOPMENTONLY) || !StrContains(name, "bot"))
        {
            continue;
        }
