...
public Action:Command_Help(const String:nick[], args) {
    decl String:description[256], String:hostmask[IRC_MAXLEN], String:channel[IRC_CHANNEL_MAXLEN];
    IRC_GetHostMask(hostmask, sizeof(hostmask));
    IRC_GetMsgChan(channel, sizeof(channel));
    
    for (new i = 0; i < GetArraySize(Commands); i++) {
        new flags = GetArrayCell(CommandPermissions, i);
        if (IRC_GetAdminFlag(hostmask, AdminFlag:flags) || (!(flags & ADMFLAG_ROOT) && IRC_IsAdminChan(channel))) {
...
HandleLine(String:prefix[], Handle:args) {
    ...
    decl String:debug2[1024];
    Format(debug2, sizeof(debug2), "%s ", prefix);
    
    new size = GetArraySize(args);
    for (new i = 0; i < size; i++) {
        decl String:temp[256];
        GetArrayString(args, i, temp, sizeof(temp));
        
        Format(debug2, sizeof(debug2), "%s%s ", debug2, temp);
    }
    
    Debug_Log("Received data: %s", debug2);
    ...
    if (StrEqual(command, "PRIVMSG")) { // Is it a privmsg? check if it's a command and then run the command.
        ...
        strcopy(g_TriggerChannel, sizeof(g_TriggerChannel), channel);
    ...
    else if (StrEqual(command, "INVITE", false)) { // Someone invited me to a channel, yay!
        decl String:channel[IRC_CHANNEL_MAXLEN];
        GetArrayString(args, 2, channel, sizeof(channel));
        
        decl String:nick[IRC_NICK_MAXLEN];
        IRC_GetNickFromHostMask(prefix, nick, sizeof(nick));
        
        if (StrEqual(nick, "ChanServ", false)) {
...
public IRC_Connected() {
    ...
    decl String:szUser[32], String:szPass[32];
    KvGetString(kv, "authuser", szUser, sizeof(szUser), "");
    KvGetString(kv, "authpass", szPass, sizeof(szPass), "");
    
    if (strlen(szUser) && strlen(szPass)) {
        IRC_Send("PRIVMSG AuthServ@Services.GameSurge.net :auth %s %s", szUser, szPass);
        IRC_Send("MODE %s +x", g_nick);
    }
    
    if (!KvJumpToKey(kv, "channels") || !KvGotoFirstSubKey(kv)) {
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max) {
    ...
    CreateNative("IRC_GetMsgChan", N_IRC_GetMsgChan);
    CreateNative("IRC_IsAdminChan", N_IRC_IsAdminChan);
...
public N_IRC_GetMsgChan(Handle:plugin, numParams) {
    SetNativeString(1, g_TriggerChannel, GetNativeCell(2));
}

public N_IRC_IsAdminChan(Handle:plugin, numParams) {
    decl String:channel[IRC_CHANNEL_MAXLEN];
    GetNativeString(1, channel, sizeof(channel));
    
    KvJumpToKey(kv, "Server");
    KvJumpToKey(kv, "channels");
    KvJumpToKey(kv, channel);
    
    new isAdminChan = KvGetNum(kv, "isadminchan");
...
Debug_Log(const String:szText[], any:...) {
    if (GetConVarBool(g_hConVarDebug)) {
        ...
        PrintToServer("*** IRC DEBUG: %s", szOutput);
    }
}
