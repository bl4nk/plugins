...
IRC_Loaded() {
    ...
    IRC_RegCmd("msg", Command_Msg, "msg <text> - Send a message to the server");
}
...
public Action:Command_Msg(const String:nick[], args) {
    decl String:text[IRC_MAXLEN], String:channel[IRC_CHANNEL_MAXLEN];
    IRC_GetCmdArgString(text, sizeof(text));
    IRC_GetMsgChan(channel, sizeof(channel));
    
    PrintToChatAll("\x04[IRC]\x03 %s\x01: %s", nick, text);
    IRC_Send("PRIVMSG %s :\x0303[IRC]\x03 %s: %s", channel, nick, text);
...
public Action:Event_PlayerSay(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (client != 0)
        ...
        team = 0;
    if (team == -1)
        Format(result, sizeof(result), "%s\x02%N\x02: %s", result, client, message);
    else
        if (client)
            Format(result, sizeof(result), "%s\x02\x03%02d%N\x03\x02: %s", result, team, client, message);
        else
            Format(result, sizeof(result), "\x02\x0307Server\x03\x02: %s", message);
...
public Action:Event_PRIVMSG(const String:hostmask[], args) {
    ...
    if (IRC_ChannelHasFlag(channel, "relay")) {
        ...
        if (strlen(text) > 5 && strncmp(text, "!msg ", 5) == 0) {
            IRC_Send("PRIVMSG %s :\x0303[IRC]\x03 %s: %s", channel, nick, text[5]);
        }
    }
}
